// All 21 Blokus pieces. squares are [row, col] coordinates, normalized so min row=0, min col=0.
export const PIECES = [
  // 1-square
...
export function getTransformedSquares(pieceId, rotation, flipped) {
  ...
  for (let i = 0; i < (rotation % 4); i++) {
    squares = rotate90CW(squares);
  }
  return squares;
}

export function getAbsoluteSquares(pieceId, rotation, flipped, row, col) {
  const transformed = getTransformedSquares(pieceId, rotation, flipped);
  return transformed.map(([r, c]) => [r + row, c + col]);
}
